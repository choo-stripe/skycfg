...
func evalAndReportResults(t *testing.T, cmd string, testCase assertTestCase) {
	...
	if (failureCtx.Failure != nil) != testCase.ExpFailure() {
		t.Errorf(
			"Assertion result (%t) did not equal expected assertion result (%t) for `%s`",
			failureCtx.Failure == nil,
			!testCase.ExpFailure(),
			cmd,
		)
		return
	...
	if testCase.ExpFailure() && !strings.Contains(failureCtx.Failure.Error(), testCase.ExpFailureMsg()) {
		t.Errorf(
			"Expected '%s' to be included in the failure message: '%s'",
			testCase.ExpFailureMsg(),
			failureCtx.Failure.Error(),
		)
	}
}
