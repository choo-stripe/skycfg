...
func (err AssertionError) Error() string {
	// straight boolean assertions like assert(false)
	if err.op == nil {
		return fmt.Sprintf("[%s] assertion failed\n%s", err.position, err.backtrace)
	}

	// binary assertions, like assert_eql
	return fmt.Sprintf("[%s] assertion failed: %s (type: %s) %s %s (type: %s)\n%s", err.position, err.val1.String(), err.val1.Type(), err.op.String(), err.val2.String(), err.val2.Type(), err.backtrace)
}
